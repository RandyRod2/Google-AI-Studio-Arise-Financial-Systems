        ...
        <div className="animate-fade-in space-y-6 relative h-full flex flex-col">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Manage your schedule, tasks, and deadlines.</p>
                </div>
                
                <div className="flex items-center gap-3">
                    <button 
                        onClick={goToToday}
                        className="px-3 py-1 text-xs font-bold bg-white border border-gray-200 rounded-lg text-slate-600 hover:bg-gray-50"
                    >
                        Today
                    </button>
                    <div className="flex items-center bg-white px-2 py-1 rounded-lg border border-gray-200 shadow-sm">
                        <button onClick={prevMonth} className="p-2 hover:bg-gray-100 rounded-md transition-colors text-slate-600">
                            <ChevronLeft size={20} />
                        </button>
                        <span className="font-bold text-slate-800 w-40 text-center select-none">
                            {monthName}
                        </span>
                        <button onClick={nextMonth} className="p-2 hover:bg-gray-100 rounded-md transition-colors text-slate-600">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 min-h-0">
                {/* Calendar Grid */}
                <div className="lg:col-span-3 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col h-[600px] lg:h-auto">
                    {/* Days Header */}
                    ...
                    <div className="grid grid-cols-7 flex-1 auto-rows-fr">
                        {calendarGrid.map((cell, index) => {
                            if (!cell) {
                                return <div key={`empty-${index}`} className="bg-gray-50/30 border-b border-r border-gray-100 last:border-r-0"></div>;
                            }
                            ...
                            return (
                                <div key={cell.dateStr} className={`border-b border-r border-gray-100 p-2 relative flex flex-col gap-1 hover:bg-gray-50/30 transition-colors last:border-r-0 ${isToday ? 'bg-indigo-50/10' : ''}`}>
                                    <div className="flex justify-between items-start">
                                        <span className={`text-sm font-semibold w-7 h-7 flex items-center justify-center rounded-full ${isToday ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-700'}`}>
                                            {cell.day}
                                        </span>
                                        {dayEvents.length > 0 && (
                                            <span className="text-[10px] text-gray-400 font-medium hidden sm:inline">{dayEvents.length} items</span>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex-1 flex flex-col gap-1 overflow-y-auto mt-1 custom-scrollbar">
                                        {dayEvents.map(event => (
                                            <div 
                                                ...
                                                ${event.type === 'Meeting' ? 'bg-indigo-50 text-indigo-700 border-indigo-500' : 
                                                  event.type === 'Call' ? 'bg-green-50 text-green-700 border-green-500' : 
                                                  event.type === 'Compliance' ? 'bg-red-50 text-red-700 border-red-500' :
                                            ...
                                            >
                                                ...
                                                <button 
                                                    ...
                                                    className="absolute right-1 top-1/2 -translate-y-1/2 hidden group-hover:block p-0.5 bg-white/50 rounded hover:bg-white text-red-600"
                                                >
                                                    <Trash2 size={10} />
                                                </button>
                                            </div>
                ...
                <div className="space-y-6 overflow-y-auto custom-scrollbar lg:h-auto">
                     <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-3">
                            {upcomingEvents.length === 0 ? (
                                ...
                                <div key={event.id} className={`border-l-4 pl-3 py-1 relative group ${event.type === 'Meeting' ? 'border-indigo-500' : event.type === 'Call' ? 'border-green-500' : 'border-gray-300'}`}>
                                    <div className="flex justify-between items-start">
                     ...
                     <div className="bg-orange-50 p-5 rounded-xl border border-orange-100">
                        ...
                        {events.filter(e => e.type === 'Compliance').slice(0, 3).map(c => (
                            <div key={c.id} className="text-xs text-orange-800 mb-2 pb-2 border-b border-orange-200 last:border-0 last:pb-0 last:mb-0">
            ...
            {isAddModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-slate-900/40 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md ring-1 ring-black/5 animate-fade-in">
                        <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-xl">
                            <h3 className="font-bold text-slate-800 flex items-center gap-2">
                                <CalendarIcon size={18} className="text-indigo-600" /> New Event
                            </h3>
