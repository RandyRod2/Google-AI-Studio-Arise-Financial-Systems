...
import { 
    ...
    AlertCircle, FileText, CheckCircle2, User
} from 'lucide-react';
...
const BookOfBusiness: React.FC<BookOfBusinessProps> = ({ clients, onUpdateClients, onViewClient }) => {
    ...
    }, [bookEntries, searchTerm, activeTab]);
    ...
    const handleAddDeal = () => {
        ...
        setNewDeal({ firstName: '', lastName: '', phone: '', carrier: '', product: '', premium: '', status: PolicyStatus.PENDING, policyNumber: '', draftDate: '' });
    };
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="flex items-center gap-2 w-full md:w-auto">
                    <div className="bg-slate-900 border border-white/10 rounded-lg p-1 flex">
                        <button className="px-3 py-1.5 text-[10px] font-bold text-white bg-slate-800 rounded-md">All</button>
                        <button className="px-3 py-1.5 text-[10px] font-bold text-slate-500 hover:text-slate-300 transition-colors">Client</button>
                    </div>
                    <button className="flex items-center gap-2 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-white transition-colors">
                        <Plus size={12} /> Filter
                    </button>
                </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    ...
                    <div className="relative">
                        ...
                        {isEditColumnsOpen && (
                            <div className="absolute right-0 top-full mt-2 w-48 bg-slate-900 border border-white/10 rounded-xl shadow-2xl z-[60] p-2 animate-in fade-in zoom-in-95">
                    ...
                    <table className="w-full text-left border-collapse min-w-[1200px]">
                        <thead className="bg-slate-950/80 sticky top-0 z-10 backdrop-blur-md">
                            <tr className="text-[9px] font-black text-slate-500 uppercase tracking-[0.15em] border-b border-white/5">
                                {visibleColumns.date && <th className="px-6 py-4">Date</th>}
                        ...
                        <tbody className="divide-y divide-white/5 bg-slate-900/20">
                            {filteredEntries.length === 0 ? (
                                ...
                                filteredEntries.map((entry) => {
                                    const nearDraft = isNearDraft(entry.policy.draftDate);
                                    return (
                                        <tr 
                                            key={entry.id} 
                                        ...
                                        >
                                            {visibleColumns.date && (
                                                <td className="px-6 py-3">
                                                    <div className="text-[10px] font-bold text-slate-300">
                                                        {new Date(entry.policy.startDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                                    </div>
                                            ...
                                            {visibleColumns.draftDate && (
                                                <td className="px-6 py-3">
                                                    <div className={`text-[10px] font-black uppercase tracking-tight flex items-center gap-1.5 ${nearDraft ? 'text-rose-500' : 'text-indigo-400'}`}>
                                                        {nearDraft && <AlertCircle size={10} className="animate-pulse" />}
                                                        {entry.policy.draftDate || 'Jan 14, 2026'}
                                                    </div>
                                                </td>
                                            )}
                                        </tr>
                    ...
                    <div className="bg-slate-900 rounded-[2rem] border border-white/10 shadow-2xl w-full max-w-xl overflow-hidden ring-1 ring-white/10 animate-in fade-in zoom-in-95 duration-300">
                        ...
                        <div className="p-8 space-y-6">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Draft Date</label>
                                    <input 
                                        type="date" 
                                        ...
                                        value={newDeal.draftDate}
                                        onChange={e => setNewDeal({...newDeal, draftDate: e.target.value})}
                                    />
                                </div>
                            </div>
