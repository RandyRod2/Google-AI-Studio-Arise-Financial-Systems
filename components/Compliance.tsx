        ...
        <div className="animate-fade-in space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-400">Manage licenses, certifications, and carrier appointments.</p>
                </div>
                <div className="flex gap-3">
                    <div className="flex items-center gap-2 bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-800 shadow-sm">
                        <ShieldCheck className="text-green-400" size={18} />
                        <span className="text-sm font-bold text-slate-200">{activeCount} Active</span>
                    </div>
                    {actionCount > 0 && (
                        <div className="flex items-center gap-2 bg-red-900/20 px-3 py-1.5 rounded-lg border border-red-900/30 shadow-sm">
                            <AlertOctagon className="text-red-400" size={18} />
                            <span className="text-sm font-bold text-red-400">{actionCount} Action Required</span>
                        </div>
                    )}
                </div>
            </div>
                ...
                <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50">
                    <div className="flex gap-2">
                        <button 
                            ...
                            className={`px-3 py-1.5 text-xs font-bold rounded-md transition-colors ${filterStatus === 'ALL' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                        >
                            All Items
                        </button>
                        <button 
                            onClick={() => setFilterStatus('ACTION')}
                            className={`px-3 py-1.5 text-xs font-bold rounded-md transition-colors ${filterStatus === 'ACTION' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                        >
                            Action Required
                        </button>
                    </div>
                    <div className="relative">
                        <Filter size={16} className="text-slate-500 absolute left-3 top-1/2 -translate-y-1/2" />
                        <input type="text" placeholder="Filter..." className="pl-9 pr-4 py-1.5 text-xs border border-slate-700 rounded-lg outline-none focus:border-indigo-500 bg-slate-950 text-white w-40 placeholder-slate-600" />
                    </div>
                </div>
                            ...
                            <tr key={item.id} className="hover:bg-slate-800/50 transition-colors group">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        <div className={`p-2 rounded-lg ${item.status === 'Active' ? 'bg-green-500/10 text-green-400' : 'bg-slate-800 text-slate-400'}`}>
                                            {item.type === 'License' ? <ShieldCheck size={18} /> : <FileText size={18} />}
                                        </div>
                                        <div>
                                            <p className="font-bold text-white text-sm">{item.name}</p>
                                            <p className="text-xs text-slate-500">ID: {item.id.toUpperCase()}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td className="px-6 py-4">
                                    <span className={`px-2.5 py-1 text-xs font-bold rounded-full inline-flex items-center gap-1
                                        ${item.status === 'Active' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 
                                          item.status === 'Expiring Soon' ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                                ...
                                <td className="px-6 py-4 text-sm font-medium text-slate-300">{item.expiry}</td>
            ...
            {complianceItems.some(i => i.name === 'AML Training' && i.status !== 'Active') && (
                <div className="bg-blue-900/20 border border-blue-500/30 p-4 rounded-xl flex items-start gap-3 animate-fade-in">
                    <div className="p-2 bg-blue-500/20 text-blue-400 rounded-full">
                        <AlertOctagon size={20} />
                    </div>
                    <div>
                        <h4 className="font-bold text-blue-400 text-sm">Anti-Money Laundering (AML) Update</h4>
            ...
            {selectedItem && (
                <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
                    <div className="bg-slate-900 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all scale-100 border border-slate-800">
                                    ...
                                    {!uploadSuccess ? (
                                        <>
                                            <div 
                                                ...
                                                className="border-2 border-dashed border-slate-700 rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500 hover:bg-indigo-900/10 transition-all group"
                                            >
                                                ...
                                                <p className="text-sm font-bold text-slate-300">Click to upload document</p>
                                        ...
                                        <div className="text-center py-4 animate-fade-in">
                                            ...
                                            <h4 className="text-xl font-bold text-white">Verification Successful</h4>
                                ...
                                <div className="space-y-6">
                                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                                        <div className="flex items-center gap-3 mb-4 pb-4 border-b border-slate-800">
                                            ...
                                            <div>
                                                <h4 className="font-bold text-white">Official Certificate</h4>
                                                <p className="text-xs text-slate-500">Verified by State Department</p>
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            <div>
                                                <p className="text-xs text-slate-500 uppercase font-bold">License Number</p>
                                                <p className="font-medium text-slate-300 mt-0.5">LIC-{Math.floor(Math.random() * 1000000)}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-slate-500 uppercase font-bold">Issue Date</p>
                                                <p className="font-medium text-slate-300 mt-0.5">Jan 15, 2023</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-slate-500 uppercase font-bold">Expiration</p>
                                                <p className="font-medium text-slate-300 mt-0.5">{selectedItem.expiry}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-slate-500 uppercase font-bold">Status</p>
                                                <span className="inline-flex items-center gap-1 text-green-400 font-bold bg-green-500/10 px-2 py-0.5 rounded text-xs mt-0.5 border border-green-500/20">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <button 
                                            onClick={closeModal}
                                            className="px-4 py-2 bg-slate-800 border border-slate-700 text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-700 hover:text-white"
                                        >
                                            Close
                                        </button>
                                    </div>
