...
import { 
    ...
    Volume2, ChevronLeft, Layout, Settings, Trash2,
    Check, History, Award, Heart, ShieldCheck, ZapIcon, Ghost, Siren,
    UserCheck, Scale, AudioLines, Stethoscope, Briefcase,
    UserPlus, Users, Train, BarChart3, PieChart, Activity,
    Network, Eye, Search
} from 'lucide-react';
import { TeamMember, Duel, Badge, Client, PolicyStatus } from '../types';
...
const Leaderboard: React.FC<LeaderboardProps> = ({ clients, teamMembers }) => {
    ...
    const [newDuel, setNewDuel] = useState<Partial<Duel>>({
        category: 'AGENT',
        competitor1Id: teamMembers[0]?.id || '',
        competitor2Id: teamMembers[1]?.id || '',
        type: 'APPS',
        target: 5,
        current1: 0,
        current2: 0,
    ...
    // --- Statistics Logic (LIVE SYNC) ---
    const allProcessedTeam = useMemo(() => {
        const now = new Date();
        const startOfPeriod = new Date(now);
        if (timeFrame === 'WEEKLY') startOfPeriod.setDate(now.getDate() - 7);
        else if (timeFrame === 'MONTHLY') startOfPeriod.setDate(1);
        else startOfPeriod.setMonth(0, 1);

        return teamMembers.map(member => {
            // Find clients belonging to this agent (or Randy mapping u1/t1)
            const agentClients = clients.filter(c => c.agentId === member.id || (member.id === 't1' && c.agentId === 'u1'));
            
            // Sum policy premiums within timeframe
            const displayProd = agentClients.reduce((sum, client) => {
                return sum + client.policies
                    .filter(p => {
                        const pDate = new Date(p.submittedDate || p.startDate);
                        return pDate >= startOfPeriod && (p.status === PolicyStatus.ACTIVE || p.status === PolicyStatus.APPROVED);
                    })
                    .reduce((pSum, p) => pSum + p.premium, 0);
            }, 0);

            // Count unique families with active policies
            const displayFamilies = agentClients.filter(c => 
                c.policies.some(p => p.status === PolicyStatus.ACTIVE || p.status === PolicyStatus.APPROVED)
            ).length;
            ...
            return {
                ...
                level: Math.floor(displayProd / 10000) + 1,
            };
        }).sort((a, b) => b.displayProduction - a.displayProduction);
    }, [timeFrame, clients, teamMembers]);
    ...
    const agencyStats = useMemo(() => {
        ...
        const activeCount = allProcessedTeam.filter(m => m.displayProduction > 0).length;
    ...
    const handleCreateDuel = () => {
        const duel: Duel = {
            ...
            competitor1Name: newDuel.category === 'AGENCY' ? newDuel.competitor1Name : getMemberById(newDuel.competitor1Id || '').name,
            competitor2Name: newDuel.category === 'AGENCY' ? newDuel.competitor2Name : getMemberById(newDuel.competitor2Id || '').name,
    ...
    const getMemberById = (id: string) => teamMembers.find(m => m.id === id) || teamMembers[0];
    const openProfile = (agent: TeamMember) => setSelectedAgentProfile(agent);
    ...
    return (
        <div className={`animate-fade-in flex flex-col transition-all duration-700 pb-12 ${isTvMode ? 'bg-slate-950 fixed inset-0 z-50 p-0 overflow-hidden' : 'gap-8 relative'}`}>
                ...
                <div className="flex flex-col gap-6 shrink-0 px-4">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                        <div className="flex items-center gap-4">
                            <div className="p-3 bg-slate-900 border border-white/10 rounded-2xl"><Trophy className="text-white" size={32} /></div>
                            <div>
                                <h2 className="font-black tracking-tighter uppercase text-3xl text-white">Agency Leaderboard</h2>
                                <p className="text-slate-500 font-bold tracking-widest text-xs uppercase flex items-center gap-2"><Flame size={14} className="text-orange-500" /> Live Arena & Standings</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                    ...
                    <div className="flex justify-end mt-2">
                        <div className="relative w-full max-w-md">
                            ...
                            <input type="text" placeholder="Search agents and view their numbers..." className="w-full pl-10 pr-4 py-3 bg-slate-900 border border-white/10 rounded-xl text-sm text-white placeholder-slate-600 focus:ring-1 focus:ring-indigo-500 outline-none transition-all shadow-xl" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className={`flex-1 transition-all duration-700 ${isTvMode ? 'flex items-center justify-center h-[calc(100vh-80px)]' : ''}`}>
                ...
                {/* 1. PODIUM VIEW */}
                {((!isTvMode && !showChampions && !searchTerm) || (isTvMode && tvTab === 'PODIUM')) && topThree.length >= 1 && (
                         ...
                         <div className="flex flex-col md:flex-row justify-center items-end gap-12 mb-8 h-[400px]">
                            {topThree[1] && (
                                <div className="flex flex-col items-center group cursor-pointer" onClick={() => openProfile(topThree[1])}>
                                    <div className="relative mb-4">
                                        <div className={`w-24 h-24 rounded-3xl border-4 border-slate-500/50 p-1 rotate-3 group-hover:rotate-0 transition-transform bg-slate-900 overflow-hidden shadow-2xl`}><img src={topThree[1].avatarUrl} alt="" className="w-full h-full object-cover grayscale-[0.3]" /></div>
                            ...
                            {topThree[0] && (
                                <div className="flex flex-col items-center z-10 group relative cursor-pointer" onClick={() => openProfile(topThree[0])}>
                                    ...
                                    <div className="relative mb-6">
                                        <div className={`w-40 h-40 rounded-3xl border-4 border-yellow-500 p-1 -rotate-3 group-hover:rotate-0 transition-transform bg-slate-900 overflow-hidden shadow-[0_0_50px_rgba(234,179,8,0.3)]`}><img src={topThree[0].avatarUrl} alt="" className="w-full h-full object-cover" /></div>
                            ...
                            {topThree[2] && (
                                <div className="flex flex-col items-center group cursor-pointer" onClick={() => openProfile(topThree[2])}>
                                    <div className="relative mb-4">
                                        <div className={`w-24 h-24 rounded-3xl border-4 border-orange-800/50 p-1 rotate-1 group-hover:rotate-0 transition-transform bg-slate-900 overflow-hidden shadow-2xl`}><img src={topThree[2].avatarUrl} alt="" className="w-full h-full object-cover grayscale-[0.4]" /></div>
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {sortedTeam.map((agent, index) => (
                                        <tr key={agent.id} onClick={() => openProfile(agent)} className="hover:bg-indigo-500/5 transition-all group cursor-pointer">
                                            ...
                                            <td className="px-6 py-5 text-center"><span className="text-sm font-black text-indigo-400">{agent.displayFamilies}</span></td>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="bg-slate-900 rounded-3xl border border-white/10 p-8 shadow-2xl relative overflow-hidden group col-span-2">
                                ...
                                <div className="flex justify-between items-center mb-8">
                                    ...
                                    {!isTvMode && <button onClick={() => setIsDuelModalOpen(true)} className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:bg-indigo-500 transition-all flex items-center gap-2 shadow-xl"><Plus size={16} /> New Duel</button>}
                                </div>
                                    ...
                                    {duels.map((duel) => {
                                        ...
                                        return (
                                            <div key={duel.id} className={`bg-slate-950 p-6 rounded-3xl border transition-all hover:border-indigo-500/40 relative overflow-hidden ${isAgency ? 'border-indigo-500/30 ring-1 ring-indigo-500/10' : 'border-white/5'}`}>
                                                {isAgency && <div className="absolute top-2 right-2 flex items-center gap-1 bg-indigo-500 text-white px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-tighter"><Network size={10} /> Leg War</div>}
                                                <div className="flex items-center justify-between relative z-10 mb-6">
                                                    <div className="flex flex-col items-center gap-3 w-1/3">
                                                        <div className="relative group cursor-pointer" onClick={() => p1 && openProfile(p1)}>
                                                            <div className={`w-20 h-20 rounded-2xl border-2 p-1 overflow-hidden transition-all bg-slate-900 ${p1Win ? 'border-yellow-500 scale-110 shadow-[0_0_20px_rgba(234,179,8,0.3)]' : 'border-slate-800'}`}>{isAgency ? <div className="w-full h-full flex items-center justify-center text-indigo-400"><Users size={32} /></div> : <img src={p1?.avatarUrl} className="w-full h-full rounded-xl object-cover" alt="" />}</div>
                                                    ...
                                                    <div className="flex flex-col items-center gap-3 w-1/3">
                                                        <div className="relative group cursor-pointer" onClick={() => p2 && openProfile(p2)}>
                                                            <div className={`w-20 h-20 rounded-2xl border-2 p-1 overflow-hidden transition-all bg-slate-900 ${p2Win ? 'border-yellow-500 scale-110 shadow-[0_0_20px_rgba(234,179,8,0.3)]' : 'border-slate-800'}`}>{isAgency ? <div className="w-full h-full flex items-center justify-center text-indigo-400"><Users size={32} /></div> : <img src={p2?.avatarUrl} className="w-full h-full rounded-xl object-cover" alt="" />}</div>
                                                ...
                                                <div className="relative h-6 bg-slate-900 rounded-full overflow-hidden border border-white/5 flex items-center justify-center">
                                                    <div className="absolute inset-0 flex"><div className={`h-full transition-all duration-700 ${p1Win ? 'bg-yellow-500' : 'bg-indigo-600'}`} style={{ width: `${(duel.current1 / (duel.current1 + duel.current2 || 1)) * 100}%` }} /><div className={`h-full transition-all duration-700 bg-rose-600`} style={{ width: `${(duel.current2 / (duel.current1 + duel.current2 || 1)) * 100}%` }} /></div>
            ...
            {isDuelModalOpen && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md">
                    <div className="bg-slate-900 rounded-3xl border border-white/10 shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
                        <div className="p-6 border-b border-white/5 bg-white/5 flex justify-between items-center"><h3 className="font-black text-white uppercase tracking-widest flex items-center gap-2"><Swords size={20} className="text-rose-500" /> Arena Configuration</h3><button onClick={() => setIsDuelModalOpen(false)} className="text-slate-400 hover:text-white transition-colors"><X size={24} /></button></div>
                        <div className="p-8 space-y-6">
                            <div><label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Matchup Category</label><div className="grid grid-cols-2 gap-2 bg-slate-950 p-1 rounded-xl border border-white/5"><button onClick={() => setNewDuel({...newDuel, category: 'AGENT'})} className={`py-2 rounded-lg text-[10px] font-black uppercase transition-all ${newDuel.category === 'AGENT' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}>Agent vs Agent</button><button onClick={() => setNewDuel({...newDuel, category: 'AGENCY'})} className={`py-2 rounded-lg text-[10px] font-black uppercase transition-all ${newDuel.category === 'AGENCY' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}>Leg vs Leg</button></div></div>
                            {newDuel.category === 'AGENT' ? <div className="grid grid-cols-2 gap-4"><div><label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Agent 1</label><select className="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-xl p-3 text-sm outline-none" value={newDuel.competitor1Id} onChange={e => setNewDuel({...newDuel, competitor1Id: e.target.value})}>{teamMembers.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div><div><label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Agent 2</label><select className="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-xl p-3 text-sm outline-none" value={newDuel.competitor2Id} onChange={e => setNewDuel({...newDuel, competitor2Id: e.target.value})}>{teamMembers.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div></div> : <div className="grid grid-cols-2 gap-4"><div><label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Agency/Leg Name 1</label><input className="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-xl p-3 text-sm outline-none" placeholder="e.g. Halpert Leg" value={newDuel.competitor1Name} onChange={e => setNewDuel({...newDuel, competitor1Name: e.target.value})} /></div><div><label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Agency/Leg Name 2</label><input className="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-xl p-3 text-sm outline-none" placeholder="e.g. Schrute Leg" value={newDuel.competitor2Name} onChange={e => setNewDuel({...newDuel, competitor2Name: e.target.value})} /></div></div>}
            ...
            {selectedAgentProfile && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-2xl animate-in fade-in duration-300 overflow-y-auto">
                    <div className="bg-slate-900 rounded-[2.5rem] border border-white/10 shadow-2xl w-full max-w-2xl overflow-hidden ring-1 ring-white/10 p-8 flex flex-col items-center relative my-8">
                        ...
                        <div className="flex flex-col items-center mb-8"><div className="relative mb-4"><img src={selectedAgentProfile.avatarUrl} className="w-32 h-32 rounded-3xl object-cover border-4 border-indigo-500 shadow-2xl" alt="" /><div className="absolute -bottom-2 -right-2 bg-indigo-600 text-white p-2 rounded-xl shadow-lg border-2 border-slate-900"><ShieldCheck size={16} /></div></div><h3 className="text-3xl font-black text-white uppercase tracking-tighter">{selectedAgentProfile.name}</h3><p className="text-xs font-black text-indigo-400 uppercase tracking-[0.3em] mt-1">{selectedAgentProfile.role.replace('_', ' ')}</p></div>
                        <div className="grid grid-cols-2 gap-4 w-full mb-8"><div className="bg-slate-950/50 p-6 rounded-3xl border border-white/5 text-center"><p className="text-[10px] font-black text-slate-500 uppercase mb-1 tracking-widest">Production (LIVE)</p><p className="text-3xl font-black text-white">${(selectedAgentProfile as any).displayProduction?.toLocaleString() || '0'}</p></div><div className="bg-slate-950/50 p-6 rounded-3xl border border-white/5 text-center"><p className="text-[10px] font-black text-slate-500 uppercase mb-1 tracking-widest">Active Families</p><p className="text-3xl font-black text-white">{(selectedAgentProfile as any).displayFamilies || '0'}</p></div></div>
                        <div className="w-full mb-8"><h4 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] border-b border-white/5 pb-2 mb-4">Trophy Room & Badges</h4><div className="flex flex-wrap gap-3">{selectedAgentProfile.badges && selectedAgentProfile.badges.length > 0 ? selectedAgentProfile.badges.map((badge, bIdx) => (<div key={bIdx} className="group relative"><div className={`flex items-center gap-2 px-3 py-2 rounded-xl border border-white/5 bg-white/5 hover:border-${badge.color}-500/50 hover:bg-${badge.color}-500/10 transition-all cursor-help`}><span className="text-xl">{badge.icon}</span><span className="text-[10px] font-black text-white uppercase tracking-tighter">{badge.label}</span></div><div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-slate-800 rounded-lg shadow-xl border border-white/10 text-[10px] text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 text-center">{badge.description}</div></div>)) : <div className="text-slate-600 text-[10px] font-bold uppercase tracking-widest italic py-4">No badges earned yet.</div>}</div></div>
