...
import { Mic, Send, Play, Square, Trophy, MessageSquare, RefreshCw, User, Bot, AlertCircle, CheckCircle2, Phone, TrendingUp, Heart, Home, Flag, BookOpen, Joystick } from 'lucide-react';
...
const TheDojo: React.FC = () => {
    ...
    return (
        <div className="animate-fade-in h-full flex flex-col">
            ...
            {/* Dojo Tab Navigation */}
            <div className="p-4 bg-white border-b border-gray-200 flex justify-center shrink-0">
                <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200 shadow-inner">
                    <button
                        onClick={() => setActiveTab('SIMULATOR')}
                        className={`flex items-center gap-2 px-6 py-2 text-sm font-bold rounded-md transition-all ${
                            activeTab === 'SIMULATOR' 
                                ? 'bg-white text-indigo-600 shadow-sm' 
                                : 'text-slate-500 hover:text-slate-700'
                        }`}
                    >
                        <Joystick size={18} /> Roleplay Simulator
                    </button>
                    <button
                    ...
                    >
                        <BookOpen size={18} /> Script Library
                    </button>
                </div>
            </div>
            ...
            {activeTab === 'SIMULATOR' && (
                <div className="flex-1 min-h-0 flex flex-col relative overflow-hidden">
                    {!selectedScenario ? (
                        ...
                        <div className="flex flex-col h-full bg-slate-100 relative w-full">
                            {/* Header */}
